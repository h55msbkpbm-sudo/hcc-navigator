#!/usr/bin/env python3
"""Restore 8 gating questions + cite-only HKCaR funnel. Does not touch decide() or Evidence cards."""
...
def patch(s):
    ...
    notes.append('q-on')
    for old, new, tag in [
        ("['age','sex','viral','metabolic','cirrhosis','afp','cardioFactors','autoimmuneFactors']",
         "['age','sex','viral','metabolic','cirrhosis','afp','lesions','largest']", 'reloc-local"),
        ("['age','sex','viral','metabolic','cirrhosis','afp']",
         "['age','sex','viral','metabolic','cirrhosis','afp','lesions','largest']", 'reloc-live'),
    ]:
        if old in s:
            s = s.replace(old, new, 1)
            notes.append(tag)
            break
    if "['albiCard','pagebCard']" in s and 'burdenCalcCard' not in s.split("[albiCard'")[0][-1:]:
        pass
    if "['albiCard','pagebCard']" in s:
        # only replace the relocate list, not other mentions
...
def main():
    for name in ('index.html', 'hcc-simulator.html'):
        ...
        assert keys == [
            'mvi', 'ehs', 'ecog', 'liver', 'varices', 'egd',
            'cardioFactors', 'autoimmuneFactors',
        ], keys
